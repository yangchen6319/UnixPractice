...
static long getNum(const char *fname, const char *arg, int flags, const char *name){
    ...
    char *endptr;
    int base;
    if (arg == NULL || *arg == '\0') gnFail(fname, "null or empty string", arg, name);

    base = (flags & GN_ANY_BASE) ? 0: (flags & GN_BASE_8) ? 8 : (flags & GN_BASE_16) ? 16 : 10;
    
    errno = 0;
    res = strtol(arg, &endptr, base);
    if (errno != 0) gnFail(fname, "strtol() fail", arg, name);
    if (*endptr != '\0') gnFail(fname, "nonnumeric characters", arg, name);
    if ((flags & GN_NO) && res < 0) gnFail(fname, "regative value not allowed", arg, name);
    if ((flags &GN_GET_0) && res <=0) gnFail(fname, "value must > 0", arg, name);
    return res;
}
...
int getInt(const char *args, int flags, const char *name){
    ...
    if (res > INT_MAX || res < INT_MIN) gnFail("getInt", "int out of range", args, name);
    return (int)res;
}
